...
# Training function for the Surround environment
def training(run):
    if run:
        env = Surround(
...
# Visualization function for the Surround environment
def visualize(run):
    if run:
        model = PPO.load("policy_surround")

        env = Surround(render_mode="human")
        env = ss.color_reduction_v0(env, mode="B")
        env = ss.resize_v1(env, x_size=84, y_size=84)
        env = ss.frame_stack_v1(env, 3)

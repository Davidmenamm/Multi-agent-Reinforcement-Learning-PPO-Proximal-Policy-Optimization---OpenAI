...
def training(run):
    if run:
        ...
        models = train_independent_models(env, num_agents, "ppo_pistonball_logs")
...
def visualize(run):
    if run:
        models = [PPO.load(f"ppo_pistonball_logs/agent_{i}/policy") for i in range(20)]  # Load models for each agent
